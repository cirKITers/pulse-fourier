...
# Important: Handle potential zeros before taking the logarithm.
# np.log(0) is undefined. A common practice is to add a small constant (epsilon)
# to all values before taking the logarithm. This ensures that even very small
# positive values are transformed meaningfully.
epsilon = 1e-10  # A very small positive number
# Using log10 is often more intuitive for visualization


# DIFFERENCE
correlation_differences = correlations_pulse - correlations_gate
diff_corr(correlation_differences, num_coeffs, num_params, 1, log=True)
...
# corr_matr(correlations_pulse, num_coeffs, num_params, 1)
